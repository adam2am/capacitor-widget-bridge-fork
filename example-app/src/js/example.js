...
window.pinWidget = async () => {
  if (Capacitor.getPlatform() !== 'android') {
    alert('Widget pinning is only available on Android!');
    return;
  }
  
  try {
    await WidgetBridgePlugin.requestWidget({ 
      className: 'com.example.plugin.MyAppWidget' // Replace with your actual widget class
    });
    alert('Widget pin request sent!');
  } catch (error) {
    alert('Error pinning widget: ' + error);
  }
};
